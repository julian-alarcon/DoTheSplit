...
- **Strategy**: precache the shell + hashed assets; runtime-cache `/v1` **GETs** stale-while-revalidate (cache `dts-v1-get`). Mutations are GET-only-excluded by Workbox and pass straight to the network; `/v1/auth/*` is excluded so tokens are never cached. `registerType: 'autoUpdate'` - a new SW applies silently on the next navigation.
