...
- Infra: Docker Compose on TrueNAS. Postgres stack: `postgres`, `migrate`, `dothesplit`. SQLite stack: a single `dothesplit` container (embedded migrations, one DB file). The `dothesplit` binary serves the API, the embedded SPA, and the in-process background worker, so there is no separate `web` or `worker` service.

## Repo Structure

```text
/server            → Go backend (single binary; also serves the embedded SPA)
/frontend          → Vue 3 SPA (built and embedded into the server binary)
...
- runs as an in-process goroutine inside the `dothesplit` server on both engines - single node by design, no separate worker process
...
- SBOMs: CycloneDX 1.5+ JSON for the `server` binary and `frontend` package; generated by `make sbom` into `/sbom/` (gitignored). On every tagged release, `.github/workflows/compliance.yml` attaches `sbom/{server,frontend}.cdx.json` plus `THIRD_PARTY_LICENSES.md` to the GitHub Release.
