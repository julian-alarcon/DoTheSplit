...
// RecurringService.Tick() the way cmd/worker/main.go does, and assert
...
func TestRecurringTickMaterializesAndAdvances(t *testing.T) {
	...
	// Run the worker's tick path directly. cmd/worker/main.go:110 calls
	// exactly this method under an advisory lock; we don't bother with the
	// lock here because the test owns the database.
