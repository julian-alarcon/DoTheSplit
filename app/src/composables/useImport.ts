...
// The full-group CSV header is `Date,Description,Category,Cost,Currency,<u1>,<u2>,...`.
// The user columns after the fixed prefix become the members to map.
...
export function memberNamesFromCsv(csv: string): string[] {
  ...
  return cols.slice(HEADER_PREFIX.length);
}
...
export async function previewGroupExpenses(
  groupId: string,
  csv: string,
): Promise<{ data: GroupExpensesResponse | null; code?: string }> {
  const { data, error, response } = await api.POST("/v1/groups/{id}/imports/expenses", {
    params: { path: { id: groupId } },
    body: { csv, dry_run: true },
  });
  if (error || !data) return { data: null, code: `http_${response.status}` };
  return { data };
}
...
export async function commitGroupExpenses(
  ...
  return error ? { ok: false, code: `http_${response.status}` } : { ok: true };
}
...
export async function previewFullImport(
  ...
  body: FullImportBody,
): Promise<{ data: SplitwiseResponse | null; code?: string }> {
...
export async function commitFullImport(
  source: "splitwise" | "dothesplit",
  body: FullImportBody,
): Promise<{ ok: boolean; groupId?: string; code?: string }> {
  const path: FullImportPath = source === "splitwise" ? "/v1/imports/splitwise" : "/v1/imports/dothesplit";
  const { data, error, response } = await api.POST(path, { body: { ...body, dry_run: false } });
  if (error || !data) return { ok: false, code: `http_${response.status}` };
