// Command server starts the DoTheSplit app: the HTTP API, the embedded SPA,
// and the in-process background worker.
package main
...
import (
	...
	"github.com/julian-alarcon/dothesplit/server/internal/crypto"
	"github.com/julian-alarcon/dothesplit/server/internal/handlers"
	"github.com/julian-alarcon/dothesplit/server/internal/realtime"
	"github.com/julian-alarcon/dothesplit/server/internal/server"
	"github.com/julian-alarcon/dothesplit/server/internal/service"
	"github.com/julian-alarcon/dothesplit/server/internal/storefactory"
	"github.com/julian-alarcon/dothesplit/server/internal/worker"
)
...
func main() {
	...
	// Background worker: the recurring/outbox tick runs as an in-process
	// goroutine. The app is single-node by design, so there is no separate
	// worker process and the tick runs unguarded.
	go worker.Run(ctx, recurringSvc, mailerSvc, 60*time.Second, logger)
