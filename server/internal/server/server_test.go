...
func newPostgresTestStore(t *testing.T) *testEngine {
	...
	// A dedicated single-connection pool for the LISTEN goroutine, mirroring cmd/server.
...
func setup(t *testing.T, opts ...setupOpt) *testStack {
	...
	// Real-time hub, mirroring cmd/server. The SQLite store publishes committed
	...
	// the same state cmd/server/main.go produces. Capture the cleartext token
	...
	// Postgres feeds the hub via a LISTEN goroutine (mirroring cmd/server); SQLite
